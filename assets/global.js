  ...
  closeMenuDrawer(event, elementToFocus = !1) {
    event !== void 0 &&
      ...
      event instanceof KeyboardEvent &&
        elementToFocus?.setAttribute("aria-expanded", !1));
  }
  onFocusOut() {
    setTimeout(() => {
...
class SlideshowComponent extends SliderComponent {
  constructor() {
    super(),
      ...
      this.sliderControlWrapper &&
        ...
        (this.announcerBarAnimationDelay = this.announcementBarSlider
          ? 250
          : 0),
  ...
  onButtonClick(event) {
    super.onButtonClick(event), (this.wasClicked = !0);
  ...
  focusInHandling(event) {
    this.sliderAutoplayButton
      ...
      : this.announcementBarSlider.contains(event.target) && this.pause();
  }
  play() {
  ...
  applyAnimationToAnnouncementBar(button = "next") {
    ...
    const itemsCount = this.sliderItems.length,
      increment = button === "next" ? 1 : -1,
      currentIndex = this.currentPage - 1;
    let nextIndex = (currentIndex + increment) % itemsCount;
    nextIndex = nextIndex === -1 ? itemsCount - 1 : nextIndex;
    const nextSlide = this.sliderItems[nextIndex],
      currentSlide = this.sliderItems[currentIndex],
      animationClassIn = "announcement-bar-slider--fade-in",
      animationClassOut = "announcement-bar-slider--fade-out",
...
class VariantSelects extends HTMLElement {
  ...
  updateSelectedSwatchValue({ target }) {
    ...
    if (tagName === "SELECT" && target.selectedOptions.length) {
      const swatchValue = target.selectedOptions[0].dataset.optionSwatchValue,
        selectedDropdownSwatchValue = this.querySelector(
    ...
    fetch(
      ...
      .then((responseText) => {
        ...
        const html = new DOMParser().parseFromString(responseText, "text/html"),
          ...
          volumePricingSource = html.getElementById(
            `Volume-${
              this.dataset.originalSection
                ? this.dataset.originalSection
                : this.dataset.section
          ...
          pricePerItemDestination = document.getElementById(
            `Price-Per-Item-${this.dataset.section}`
          ),
          pricePerItemSource = html.getElementById(
            `Price-Per-Item-${
              this.dataset.originalSection
                ? this.dataset.originalSection
                : this.dataset.section
          ...
          volumePricingDestination = document.getElementById(
            `Volume-${this.dataset.section}`
          ),
          qtyRules = document.getElementById(
            `Quantity-Rules-${this.dataset.section}`
          ),
          volumeNote = document.getElementById(
            `Volume-Note-${this.dataset.section}`
          );
        volumeNote && volumeNote.classList.remove("hidden"),
          volumePricingDestination &&
            volumePricingDestination.classList.remove("hidden"),
          qtyRules && qtyRules.classList.remove("hidden"),
        ...
        price && price.classList.remove("hidden"),
          inventoryDestination &&
  ...
  setUnavailable() {
    const button = document.getElementById(
      ...
      volumeTable = document.getElementById(`Volume-${this.dataset.section}`),
      qtyRules = document.getElementById(
        `Quantity-Rules-${this.dataset.section}`
      );
    addButton &&
      ((addButtonText.textContent = window.variantStrings.unavailable),
      price && price.classList.add("hidden"),
      inventory && inventory.classList.add("hidden"),
      sku && sku.classList.add("hidden"),
      pricePerItem && pricePerItem.classList.add("hidden"),
      volumeNote && volumeNote.classList.add("hidden"),
      volumeTable && volumeTable.classList.add("hidden"),
      qtyRules && qtyRules.classList.add("hidden"));
  }
  getVariantData() {
    return (
      (this.variantData =
...
//# sourceMappingURL=/s/files/1/0282/8425/6353/t/173/assets/global.js.map?v=1711693782
